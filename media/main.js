...
(function () {
    ...
    function firstZoom() {
        ...
        scale = image.clientWidth / (image.naturalWidth || 1);
        updateScale(scale);
    }
    function zoomIn() {
        if (scale === 'fit') {
    ...
    container.addEventListener('wheel', (e) => {
        ...
        updateScale((scale === 'fit' ? 1 : scale) * (1 - delta * SCALE_PINCH_FACTOR));
    }, { passive: false });
    ...
    window.addEventListener('touchstart', (e) => {
        ...
        if (e.touches && e.touches.length === 2) {
            ...
            pinchActive = true;
            pinchStartScale = (scale === 'fit') ? (image.clientWidth / (image.naturalWidth || 1)) : scale;
            panStart = undefined;
            isDragging = false;
            return;
        }
    ...
    image.addEventListener('load', () => {
        if (hasLoadedImage)
            return;
        hasLoadedImage = true;
        vscode.postMessage({ type: 'size', value: `${image.naturalWidth}x${image.naturalHeight}` });
    ...
    image.addEventListener('error', () => { if (hasLoadedImage)
        return; hasLoadedImage = true; document.body.classList.add('error'); document.body.classList.remove('loading'); });
